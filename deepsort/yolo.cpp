...
std::vector<Ort::Box> Ort::YOLO::detect(const cv::Mat &image, float conf_th, float iou_th, int maxdet)
{
    ...
    {
    [[likely]] case ONNXTensorElementDataType::ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16:
    ...
    for (auto i : indices)
    {
        // if (myclsid[i] >= 3) continue;
        auto &box = boxes[i];
        // std::cout << conf_vec[i] << " ";
        if (myclsid[i] <= 3)
            result.emplace_back(myclsid[i], conf_vec[i], box.x, box.y, box.x + box.width, box.y + box.height);
    }
    // std::cout << std::endl;
